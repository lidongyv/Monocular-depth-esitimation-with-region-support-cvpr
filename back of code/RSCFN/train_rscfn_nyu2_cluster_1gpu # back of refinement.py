...
# @Last Modified time: 2018-12-15 15:46:23
import sys
import torch
import visdom
import argparse
...
def train(args):
    ...
    if args.visdom:
        vis = visdom.Visdom(env='nyu_resample_depth_')


        resample_depth_window = vis.image(
    ...
    if args.resume is not None:
        ...
        rsn=torch.load('/home/lidong/Documents/RSCFN/depth_augment_rsn_cluster_nyu_40_0.6688848_coarse_best_model.pkl',map_location='cpu')
        model_dict=model.state_dict()  
        #print(model_dict)          
    ...
    #for epoch in range(0, args.n_epoch):
        ...
        for i, (images, labels,regions,segments,image) in enumerate(trainloader):
            ...
            while(True):
                ...
                lin_d=torch.mean(torch.sqrt(torch.sum(torch.where(mask>0,torch.pow(torch.log10(accurate)-torch.log10(labels),2),mask).view(labels.shape[0],-1),dim=-1)/torch.sum(mask.view(labels.shape[0],-1),dim=-1)))
            ...
            if (i+1)%(train_len//2)==0:
                ...
                for i_val, (images_val, labels_val,regions,segments,images) in tqdm(enumerate(valloader)):
                    ...
                    with torch.no_grad():
                        ...
                        lin_d=torch.mean(torch.sqrt(torch.sum(torch.where(mask>0,torch.pow(torch.log10(depth)-torch.log10(labels_val),2),mask).view(labels_val.shape[0],-1),dim=-1)/torch.sum(mask.view(labels_val.shape[0],-1),dim=-1)))
                        #accurate=torch.where(accurate>torch.mean(accurate)*4,torch.mean(accurate)*4,accurate)
                        #depth=torch.where(depth>torch.mean(depth)*4,torch.mean(accurate)*4,depth)
                ...
                if error<= best_error:
                    ...
                    torch.save(state, "resample_depth_{}_{}_{}_{}_coarse_best_model.pkl".format(
        ...
        if epoch%check==0:
            ...
            for i_val, (images_val, labels_val,regions,segments,images) in tqdm(enumerate(valloader)):
                ...
                with torch.no_grad():
                    ...
                    lin_d=torch.mean(torch.sqrt(torch.sum(torch.where(mask>0,torch.pow(torch.log10(accurate)-torch.log10(labels_val),2),mask).view(labels_val.shape[0],-1),dim=-1)/torch.sum(mask.view(labels_val.shape[0],-1),dim=-1)))
        ...
        if epoch%3==0:
            ...
            state = {'epoch': epoch+1,
                     ...
                     'optimizer_state': optimizer.state_dict(), 
                     'error': error,}
            torch.save(state, "resample_depth_{}_{}_{}_coarse_model.pkl".format(
                args.arch, args.dataset,str(epoch)))
            print('save success')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume', nargs='?', type=str, default='/home/lidong/Documents/RSCFN/resample_depth_rsn_cluster_nyu_42_0.6494135_coarse_best_model.pkl',
                        help='Path to previous saved model to restart from /home/lidong/Documents/RSCFN/resample_depth_rsn_cluster_nyu_43_0.7498353_coarse_best_model.pkl')
