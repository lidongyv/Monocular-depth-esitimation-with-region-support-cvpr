...
# @Last Modified time: 2018-12-18 14:43:16
import sys
import torch
import visdom
import argparse
...
def train(args):
    ...
    if args.visdom:
        vis = visdom.Visdom(env='nyu_inrefine2_depth')


        inrefine2_depth_window = vis.image(
    ...
    #for epoch in range(0, args.n_epoch):
        ...
        for i, (images, labels,regions,segments,image) in enumerate(trainloader):
            ...
            while(True):
                ...
                loss_v=v_loss(variance,accurate,depth,label,mask)
                print('mean_variance:%.4f,max_variance:%.4f'%(torch.mean(accurate-depth).item(),torch.max(accurate-depth).item()))
                ...
                # else:
                #     loss=loss_a
                loss=0.2*loss_d+0.4*loss_a+0.4*loss_v
                #loss=loss_a
                #mask=mask.float()
                #mask=(labels>alpha)&(labels<beta)&(labels<torch.max(labels))&(labels>torch.min(labels))
                ...
                lin=torch.sum(torch.sqrt(torch.sum(torch.where(mask>0,torch.pow(accurate-labels,2),mask).view(labels.shape[0],-1),dim=-1)/torch.sum(mask.view(labels.shape[0],-1),dim=-1)))/labels.shape[0]
                ...
                lin_d=torch.sum(torch.sum(torch.where(mask>0,torch.abs(torch.log10(accurate)-torch.log10(labels)),mask).view(labels.shape[0],-1),dim=-1)/torch.sum(mask.view(labels.shape[0],-1),dim=-1))/labels.shape[0]
            ...
            if (i+1)%(train_len//2)==0:
                ...
                for i_val, (images_val, labels_val,regions,segments,images) in tqdm(enumerate(valloader)):
                    ...
                    with torch.no_grad():
                        ...
                        mask=(labels_val>alpha)&(labels_val<beta)&(labels_val>0)&(labels_val<torch.max(labels_val))&(labels_val>torch.min(labels_val))
                        mask=mask.float()
                        depth=depth[...,6:-6,8:-8]
                ...
                if error<= best_error:
                    ...
                    torch.save(state, "inrefine2_depth_{}_{}_{}_{}_coarse_best_model.pkl".format(
        ...
        if epoch%check==0:
            ...
            for i_val, (images_val, labels_val,regions,segments,images) in tqdm(enumerate(valloader)):
                ...
                with torch.no_grad():
                    ...
                    # loss=loss+loss_d
                    scale=1
                    labels_val=labels_val.squeeze()[...,scale*6:-6*scale,scale*8:-8*scale]
                    #regions_val=regions_val.view_as(depth)[...,6:-6,8:-8]
                    #accurate=torch.where(accurate>torch.mean(accurate)*4,torch.mean(accurate),accurate)
                    mask=(labels_val>alpha)&(labels_val<beta)&(labels_val>0)&(labels_val<torch.max(labels_val))&(labels_val>torch.min(labels_val))
                    mask=mask.float()
                    depth=depth[...,6:-6,8:-8]
                    #accurate=F.interpolate(accurate,scale_factor=2,mode='bilinear',align_corners=False).squeeze()
                    accurate=accurate[...,scale*6:-6*scale,scale*8:-8*scale]
                    images=images[...,6:-6,8:-8]
                    accurate=torch.where(accurate>beta,beta*one,accurate)
                    accurate=torch.where(accurate<alpha,alpha*one,accurate)
                    #lin=torch.sqrt(torch.mean(torch.pow(accurate[mask]-labels_val[mask],2)))
                    ...
                    #print(loss_ave[-1])
                    #print(torch.max(torch.abs(accurate[mask]-labels_val[mask])).item(),torch.min(torch.abs(accurate[mask]-labels_val[mask])).item())
                    print("error=%.4f,error_d=%.4f"%(lin.item()/labels_val.shape[0],lin_d.item()/labels_val.shape[0]))
                    # print("loss_d=%.4f loss_var=%.4f loss_dis=%.4f loss_reg=%.4f"%(torch.sum(lin).item()/4,torch.sum(loss_var).item()/4, \
                    #             torch.sum(loss_dis).item()/4,0.001*torch.sum(loss_reg).item()/4))
                if args.visdom:
                    vis.line(
        ...
        if epoch%3==0:
            ...
            state = {'epoch': epoch+1,
                     ...
                     'optimizer_state': optimizer.state_dict(), 
                     'error': error,}
            torch.save(state, "inrefine2_depth_{}_{}_{}_coarse_model.pkl".format(
                args.arch, args.dataset,str(epoch)))
            print('save success')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume', nargs='?', type=str, default='/home/lidong/Documents/RSCFN/resample_depth_rsn_cluster_nyu_84_0.62207866_coarse_best_model.pkl',
                        help='Path to previous saved model to restart from /home/lidong/Documents/RSCFN/inrefine2_depth_rsn_cluster_nyu_3_coarse_model.pkl')
