...
def train(args):
    ...
    if args.visdom:
        vis = visdom.Visdom(env='nyu2_cluster')
    ...
    if args.resume is not None:
        ...
        rsn=torch.load('/home/lidong/Documents/RSCFN/rsn_cluster_nyu2_1_pretrain_best_model.pkl',map_location='cpu')
        model_dict=model.state_dict()  
        #print(model_dict)          
    ...
    #for epoch in range(0, args.n_epoch):
        ...
        for i, (images, labels,regions,segments) in enumerate(trainloader):
            ...
            depth,loss_var,loss_dis,loss_reg = model(images,segments,1,'test')
            ...
            # loss_d=berhu(depth,labels)
            lin=torch.sqrt(torch.mean(torch.pow(depth-labels,2)))
            # loss=loss_d
            loss.backward()
            optimizer.step()
            ...
            if args.visdom:
                with torch.no_grad():
                    ...
                    # region = masks.data.cpu().numpy().astype('float32')
                    # region = region[0,...]
                    # region = (np.reshape(region, [480, 640]).astype('float32'))/(np.max(region)+0.001)
                    # vis.image(
                    #     region,
                    #     opts=dict(title='region!', caption='region.'),
                    #     win=region_window,
                    ...
                    ground = ground[0, :, :]
                    ground = (np.reshape(ground, [480, 640]).astype('float32'))/(np.max(ground)+0.001)
                    vis.image(
                        ground,
                        opts=dict(title='ground!', caption='ground.'),
                        win=ground_window,
                    ...
                    depth = (np.reshape(depth, [480, 640]).astype('float32'))/(np.max(depth)+0.001)
                    vis.image(
                        depth,
                        opts=dict(title='depth!', caption='depth.'),
                        win=depth_window,
            ...
            print("data [%d/199/%d/%d] Loss: %.4f d: %.4f loss_d:%.4f, loss_var: %.4f loss_dis: %.4f loss_reg: %.4f " % (i, epoch, args.n_epoch,loss.item(),torch.sum(lin).item(), loss_d.item(), \
        ...
        if epoch%check==0:
            ...
            for i_val, (images_val, labels_val,regions,segments) in tqdm(enumerate(valloader)):
                ...
                with torch.no_grad():
                    ...
                    depth = model(images_val,segments_val,1,'eval')
                    # loss_d=berhu(depth,labels_val)
                    # loss=torch.sum(loss_var)+torch.sum(loss_dis)+0.001*torch.sum(loss_reg)
                    # loss=loss+loss_d
                    lin=torch.sqrt(torch.mean(torch.pow(depth-labels_val,2)))
                    loss_ave.append(lin.data.cpu().numpy())
                    #print('error:')
                    #print(loss_ave[-1])
                    print("error=%.4f"%(torch.sum(lin).item()))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume', nargs='?', type=str, default='/home/lidong/Documents/RSCFN/rsn_cluster_nyu2_100_model.pkl',
                        help='Path to previous saved model to restart from /home/lidong/Documents/RSCFN/depth_rsn_cluster_nyu2_best_model.pkl')
