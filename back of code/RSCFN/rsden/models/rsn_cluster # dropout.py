...
# @Last Modified time: 2018-12-21 22:37:10
...
class rsn_cluster(nn.Module):
    ...
    def __init__(self, 
        ...
        self.fuse3 = conv2DGroupNormRelu(in_channels=128, k_size=3, n_filters=64,
                                                 padding=1, stride=1, bias=False,group_dim=group_dim) 
        self.fuse4 = conv2DGroupNormRelu(in_channels=128, k_size=3, n_filters=64,
                                                 padding=1, stride=1, bias=False,group_dim=group_dim) 
        self.inplanes = 64
        #self.regress1 = self._make_layer(block,128, 4, stride=1)
        self.regress1 = conv2DGroupNormRelu(in_channels=64, k_size=3, n_filters=32,
                                                  padding=1, stride=1, bias=False,group_dim=group_dim)
        self.regress2 = conv2DGroupNormRelu(in_channels=32, k_size=3, n_filters=16,
                                                  padding=1, stride=1, bias=False,group_dim=16)
        self.regress3 = conv2DRelu(in_channels=16, k_size=3, n_filters=16,
                                                 padding=1, stride=1, bias=False) 
        # self.regress4 = conv2DRelu(in_channels=64, k_size=1, n_filters=32,
        ...
        #                                          padding=1, stride=1, bias=False,group_dim=16)
        self.inrefine4= conv2D(in_channels=16, k_size=1, n_filters=8,
                                                 padding=0, stride=1, bias=False)
    ...
    def forward(self, x,segments,labels,flag,task):
        ...
        if flag==0:
            ...
            x=torch.nn.functional.dropout2d(x,p=0.2)
            #print(x.shape)
            x=self.regress2(x)
            x=torch.nn.functional.dropout2d(x,p=0.1)
            x=self.regress3(x)
            # x=self.regress4(x)
            ...
            # x_fuse=torch.cat([initial_depth,x_share],1)
            # x=self.inrefine1(x_fuse)
            # #print(x.shape)
            # x=self.inrefine2(x)
            # x=self.inrefine3(x)
            # x=self.inrefine4(x)
            # variance=self.inrefine5(x)
            # accurate_depth=initial_depth+variance
