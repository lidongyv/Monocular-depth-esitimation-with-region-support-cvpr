...
# @Last Modified time: 2018-11-12 23:53:19
...
class rsn_cluster(nn.Module):
    ...
    def __init__(self, 
        ...
        self.fuse2 = conv2DGroupNormRelu(in_channels=128, k_size=3, n_filters=128,
                                                 padding=1, stride=1, bias=False,group_dim=group_dim)
        ...
        self.fuse3 = conv2DGroupNormRelu(in_channels=128, k_size=3, n_filters=64,
                                                 padding=1, stride=1, bias=False,group_dim=group_dim) 
        self.fuse4 = conv2DGroupNormRelu(in_channels=64, k_size=3, n_filters=64,
                                                 padding=1, stride=1, bias=False,group_dim=group_dim) 
        self.inplanes = 64
        ...
        self.regress1 = conv2DGroupNormRelu(in_channels=64, k_size=3, n_filters=64,
                                                  padding=1, stride=1, bias=False,group_dim=group_dim)
        self.regress2 = conv2DGroupNormRelu(in_channels=64, k_size=3, n_filters=64,
                                                  padding=1, stride=1, bias=False,group_dim=group_dim)
        self.regress3 = conv2DGroupNormRelu(in_channels=64, k_size=3, n_filters=32,
                                                 padding=1, stride=1, bias=False,group_dim=group_dim) 
        self.regress4 = conv2DRelu(in_channels=32, k_size=3, n_filters=16,
                                         padding=1, stride=1, bias=False)
        self.regress5 = conv2DRelu(in_channels=16, k_size=3, n_filters=1,
                                         padding=1, stride=1, bias=False)       
    ...
    def forward(self, x,segments,labels,flag,task):
        ...
        if flag==0:
            ...
            return depth,depth,one,one,one
        else:
            if task=='train':
                with torch.no_grad():
                    masks=fast_cluster(y).view_as(depth)
