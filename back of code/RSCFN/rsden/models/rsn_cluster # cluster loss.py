...
# @Last Modified time: 2018-12-26 15:51:41
...
class rsn_cluster(nn.Module):
    ...
    def __init__(self, 
        ...
        self.regress5 = conv2DRelu(in_channels=16, k_size=1, n_filters=1,
                                         padding=0, stride=1, bias=False)       
        self.class0= conv2DGroupNormRelu(in_channels=128, k_size=3, n_filters=64,
                                                 padding=1, stride=1, bias=False,group_dim=group_dim)
    ...
    def forward(self, x,segments,labels,flag,task):
        ...
        if flag==0:
            ...
            with torch.no_grad():
                masks=fast_cluster(y).view_as(labels)
            loss_var,loss_dis,loss_reg = cluster_loss(y,masks.long(),labels)
