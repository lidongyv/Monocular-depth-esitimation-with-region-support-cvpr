...
# @Last Modified time: 2018-11-06 00:50:50
...
class rsn_cluster(nn.Module):
    ...
    def forward(self, x,segments,labels,flag,task,dis=0.16):
        #print(x.shape)
        ...
        if flag==0:
            ...
            y=self.class4(y)
            with torch.no_grad():
                masks=fast_cluster(y,bandwidth=dis).view(1,1,x_share.shape[-2],x_share.shape[-1])
