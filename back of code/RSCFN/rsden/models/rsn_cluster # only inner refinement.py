...
# @Last Modified time: 2018-10-31 16:00:00
...
class rsn_cluster(nn.Module):
    ...
    def __init__(self, 
        ...
        self.outrefine1=conv2DGroupNormRelu(in_channels=518, k_size=1, n_filters=128,
                                                 padding=0, stride=1, bias=False,group_dim=group_dim)
        ...
        self.outrefine4= conv2DRelu(in_channels=32, k_size=1, n_filters=1,
                                                 padding=0, stride=1, bias=False)
        self.inrefine1=conv2DGroupNormRelu(in_channels=259, k_size=3, n_filters=128,
                                                 padding=1, stride=1, bias=False,group_dim=group_dim)
    ...
    def forward(self, x,segments,flag,task):
        ...
        if flag==0:
            ...
            if task=='train':
                ...
                mean_depth=torch.zeros(torch.max(masks).long()-torch.min(masks).long()+1).cuda(cuda_id)
