...
# @Last Modified time: 2018-11-04 21:42:04
...
class rsn_cluster(nn.Module):
    ...
    def __init__(self, 
        ...
        self.outrefine4= conv2D(in_channels=32, k_size=1, n_filters=1,
                                                 padding=0, stride=1, bias=False)
        self.inrefine1=conv2DGroupNormRelu(in_channels=515, k_size=3, n_filters=128,
                                                 padding=1, stride=1, bias=False,group_dim=group_dim)
    ...
    def forward(self, x,segments,labels,flag,task):
        ...
        if flag==0:
            ...
            x=self.regress1(torch.cat([x_share,masks/torch.max(masks),location_map],1))
            ...
            inner_feature= torch.cat([coarse_depth,x_share,coarse_feature,location_map],1)
