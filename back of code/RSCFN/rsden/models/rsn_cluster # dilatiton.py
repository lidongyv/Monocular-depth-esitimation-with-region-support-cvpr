...
# @Last Modified time: 2018-12-08 16:06:08
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None,dilation=1):
        ...
        self.gn2 = nn.GroupNorm(group_dim,planes)
        self.conv3 = nn.Conv2d(planes, planes * expansion, kernel_size=1, bias=False,dilation=dilation)
        self.gn3 = nn.GroupNorm(group_dim,planes * expansion)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
...
class rsn_cluster(nn.Module):
    ...
    def __init__(self, 
        ...
        block=Bottleneck
        # Encoder
        self.conv1=conv2DGroupNormRelu(3, 32, k_size=7,
                                                padding=3, stride=1, bias=False,dilation=1)
        self.conv2=conv2DGroupNormRelu(32, 64, k_size=3,
                                                padding=1, stride=1, bias=False,dilation=1)       
        ...
        self.pyramid_pooling = AtrouspyramidPoolingGroupNorm(2048, aspp_dilation,group_dim=group_dim,kernel=aspp_kernel)
        ...
        self.fuse0 = conv2DGroupNormRelu(in_channels=1280, k_size=1, n_filters=512,
                                                padding=0, stride=1, bias=False,group_dim=group_dim)        
        # self.fuse1 = conv2DGroupNormRelu(in_channels=512, k_size=3, n_filters=256,
        #                                          padding=1, stride=1, bias=False,group_dim=group_dim)
        #we need to replace the upsampling unit with nearest and deconv2d
        self.deconv1 = deconv2DGroupNormRelu(in_channels=512, n_filters=512, k_size=4, 
                                                 stride=2, padding=1,output_padding=0, bias=False,group_dim=group_dim)
        self.fuse2 = conv2DGroupNormRelu(in_channels=1024, k_size=1, n_filters=256,
                                                 padding=0, stride=1, bias=False,group_dim=group_dim)
        self.deconv2 = deconv2DGroupNormRelu(in_channels=256, n_filters=256, k_size=4, 
                                                 stride=2, padding=1,output_padding=0, bias=False,group_dim=group_dim)
        # self.fuse3 = conv2DGroupNormRelu(in_channels=512, k_size=1, n_filters=128,
        #                                          padding=0, stride=1, bias=False,group_dim=group_dim) 
        self.fuse4 = conv2DGroupNormRelu(in_channels=512, k_size=1, n_filters=128,
                                                 padding=0, stride=1, bias=False,group_dim=group_dim) 
        self.inplanes = 64
        ...
        self.reliable3= conv2DGroupNormRelu(in_channels=64, k_size=1, n_filters=32,
                                                 padding=0, stride=1, bias=False,group_dim=group_dim)    
        self.reliable4= conv2D(in_channels=32, k_size=1, n_filters=1,
                                                 padding=0, stride=1, bias=False)
    ...
    def _make_MG_unit(self, block, planes, blocks, stride=1,dilation=1):
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride,downsample,dilation=blocks[0]*dilation))
        self.inplanes = planes * block.expansion
    ...
    def forward(self, x,segments,labels,flag,task):
        ...
        #print(x.shape)
        #x = self.fuse3(x)
        x_share = self.fuse4(torch.cat([x,x1],1))
        #print(x_share.shape)
        #exit()
        ...
        if flag==0:
            ...
            # inner_feature= torch.cat([coarse_depth,torch.pow(x_fuse-coarse_feature,2)],1)
            ...
            # reliable_feature= torch.cat([depth,torch.pow(x_fuse-coarse_feature,2)],1)
            # reliable=self.reliable1(reliable_feature)
            # reliable=self.reliable2(reliable)
            # reliable=self.reliable3(reliable)
            # reliable_variance=self.reliable4(reliable)
            # #reliable_variance=self.reliable5(reliable)
            ...
            #print(torch.mean(reliable_variance).item(),torch.mean(inner_variance).item(),torch.mean(torch.abs(depth-accurate_depth)).item())
            # accurate_depth=torch.where(masks>0,(depth+accurate_depth)/2,initial_depth)
