...
# @Last Modified time: 2018-11-12 21:29:38
...
class NYU2(data.Dataset):
    def __init__(self, root, split="train", is_transform=True, img_size=(480,640),task='depth'):
        ...
        self.nyu2=len(self.files)
        if split=='train':
            self.files=self.files+os.listdir('/home/dataset2/nyu/nyu1/train/')
    ...
    def __getitem__(self, index):
        ...
        # initial_region=np.arange(1,np.max(region)+1)
        # np.random.shuffle(initial_region)
        # for i in range(1,int(np.max(region))+1):
        #     region=np.where(region==i,-initial_region[i-1],region)
