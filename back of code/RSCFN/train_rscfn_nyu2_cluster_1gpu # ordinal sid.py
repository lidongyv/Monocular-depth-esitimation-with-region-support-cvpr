...
# @Last Modified time: 2018-12-06 10:17:00
import sys
import torch
import visdom
import argparse
...
def train(args):
    ...
    if args.visdom:
        vis = visdom.Visdom(env='nyu_sid_ordinal_')


        sid_ordinal_window = vis.image(
    ...
    if args.resume is not None:
        ...
        rsn=torch.load('/home/lidong/Documents/RSCFN/sid_ordinal_rsn_cluster_nyu_24_0.73801327_coarse_best_model.pkl',map_location='cpu')
        model_dict=model.state_dict()  
        #print(model_dict)          
    ...
    #for epoch in range(0, args.n_epoch):
        ...
        for i, (images, labels,regions,segments,image) in enumerate(trainloader):
            ...
            while(True):
                ...
                mask=(labels>alphat)&(labels<beta)&(labels<torch.max(labels))&(labels>torch.min(labels))
                mask=mask.float()
                ...
                lin_d=torch.mean(torch.sqrt(torch.sum(torch.where(mask>0,torch.pow(torch.log10(depth)-torch.log10(labels),2),mask).view(labels.shape[0],-1),dim=-1)/torch.sum(mask.view(labels.shape[0],-1),dim=-1)))
            ...
            if args.visdom:
                with torch.no_grad():
                    ...
                    depth = (np.reshape(depth, [228, 304]).astype('float32'))/(np.max(depth)+0.001)
                    vis.image(
                        depth,
                        opts=dict(title='depth!', caption='depth.'),
                        win=sid_ordinal_window,
                    )
            ...
            if (i+1)%(train_len//2)==0:
                ...
                if error<= best_error:
                    ...
                    torch.save(state, "sid_ordinal_{}_{}_{}_{}_coarse_best_model.pkl".format(
        ...
        if epoch%3==0:
            ...
            state = {'epoch': epoch+1,
                     ...
                     'optimizer_state': optimizer.state_dict(), 
                     'error': error,}
            torch.save(state, "sid_ordinal_{}_{}_{}_coarse_model.pkl".format(
                args.arch, args.dataset,str(epoch)))
            print('save success')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume', nargs='?', type=str, default='/home/lidong/Documents/RSCFN/sid_ordinal_rsn_cluster_nyu_27_0.66811305_coarse_best_model.pkl',
                        help='Path to previous saved model to restart from /home/lidong/Documents/RSCFN/sid_ordinal_rsn_cluster_nyu_11_0.87275344_coarse_best_model.pkl')
