...
# @Last Modified time: 2018-12-04 20:15:28
import sys
import torch
import visdom
import argparse
...
def train(args):
    ...
    if args.resume is not None:
        ...
        rsn=torch.load('/home/lidong/Documents/RSCFN/depth_rsn_cluster_nyu_14_0.79439026_coarse_best_model.pkl',map_location='cpu')
        model_dict=model.state_dict()  
        #print(model_dict)          
    ...
    #for epoch in range(0, args.n_epoch):
        ...
        for i, (images, labels,regions,segments,image) in enumerate(trainloader):
            ...
            while(True):
                ...
                mask=(labels>0)&(labels<torch.max(labels))&(labels>torch.min(labels))
                ...
                loss.backward()
                optimizer.step()
                if lin<0.5 or iterative_count>10:
                    break
                else:
                    iterative_count+=1
                    print("repeat data [%d/%d/%d/%d] Loss: %.4f lin: %.4f lin_d:%.4f loss_d:%.4f loss_a:%.4f loss_var:%.4f loss_dis:%.4f loss_reg: %.4f" % (i,train_len, epoch, args.n_epoch,loss.item(),lin.item(),lin_d.item(), loss_d.item(),loss_a.item(), \
            ...
            if (i+1)%3000==0:
                ...
                for i_val, (images_val, labels_val,regions,segments,image) in tqdm(enumerate(valloader)):
                    ...
                    with torch.no_grad():
                        ...
                        mask=(labels_val>0)&(labels_val<torch.max(labels_val))&(labels_val>torch.min(labels_val))
...
if _name_ == '_main_':
    ...
    parser.add_argument('--resume', nargs='?', type=str, default='/home/lidong/Documents/RSCFN/sid_ordinal_rsn_cluster_nyu_24_0.73801327_coarse_best_model.pkl',
