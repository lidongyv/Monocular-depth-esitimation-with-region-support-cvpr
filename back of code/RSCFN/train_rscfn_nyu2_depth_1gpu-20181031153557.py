...
# @Last Modified time: 2018-10-31 12:34:35
import sys
import torch
import visdom
import argparse
...
def train(args):
    ...
    if args.resume is not None:
        ...
        rsn=torch.load('/home/lidong/Documents/RSCFN/rsn_cluster_nyu2_40_1.126074coarse_best_model.pkl',map_location='cpu')
        model_dict=model.state_dict()  
        #print(model_dict)          
    ...
    #for epoch in range(0, args.n_epoch):
        ...
        for i, (images, labels,regions,segments,image) in enumerate(trainloader):
            ...
            loss_a=log_loss(accurate,labels)
            #loss_d=log_loss(depth,labels)
            #loss=log_loss(depth, labels)
            loss=0.5*loss_d+loss_a
            ...
            if args.visdom:
                with torch.no_grad():
                    ...
                    vis.line(
                        X=torch.ones(1).cpu() * i+torch.ones(1).cpu() *(epoch-trained)*795,
            ...
            loss_rec.append([i+epoch*795,torch.Tensor([loss.item()]).unsqueeze(0).cpu()])
            loss_error+=lin.item()
            print("data [%d/795/%d/%d] Loss: %.4f d: %.4f loss_d:%.4f loss_a:%.4f" % (i, epoch, args.n_epoch,loss.item(),lin.item(), loss_d.item(),loss_a.item()))
            # print("data [%d/795/%d/%d] Loss: %.4f linear: %.4f " % (i, epoch, args.n_epoch,loss.item(),lin.item()
            #                    ))
        print("loss=%.2f"%(loss_error/795))
        # state = {'epoch': epoch+1,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume', nargs='?', type=str, default='/home/lidong/Documents/RSCFN/rsn_cluster_nyu2_15_1.1132934coarse_best_model.pkl',
                        help='Path to previous saved model to restart from /home/lidong/Documents/RSCFN/rsn_cluster_nyu2_15_1.1132934coarse_best_model.pkl')
