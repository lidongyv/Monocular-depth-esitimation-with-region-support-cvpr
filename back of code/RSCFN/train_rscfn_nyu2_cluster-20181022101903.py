...
# @Last Modified time: 2018-10-22 09:34:28
import sys
import torch
import visdom
import argparse
...
def train(args):
    ...
    if args.resume is not None:
        ...
        rsn=torch.load('/home/lidong/Documents/RSCFN/rsn_cluster_nyu2_best_model.pkl',map_location='cpu')
        model_dict=model.state_dict()  
        #print(model_dict)          
    ...
    #for epoch in range(0, args.n_epoch):
        ...
        for i, (images, labels,regions,segments) in enumerate(trainloader):
            ...
            depth,loss_var,loss_dis,loss_reg = model(images,segments,1)
            ...
            if args.visdom:
                with torch.no_grad():
                    ...
                    depth = depth[0, :, :, :]
                    depth = (np.reshape(depth, [120, 160]).astype('float32')-np.min(depth))/(np.max(depth)-np.min(depth)+1)
                    vis.image(
                        depth,
                        opts=dict(title='depth!', caption='depth.'),
                        win=depth_window,
                    ...
                    # region = region[0,...]
                    # region = (np.reshape(region, [120, 160]).astype('float32')-np.min(region))/(np.max(region)-np.min(region)+1)
                    # vis.image(
                    #     region,
                    #     opts=dict(title='region!', caption='region.'),
                    #     win=region_window,
                    ...
                    ground = ground[0, :, :]
                    ground = (np.reshape(ground, [120, 160]).astype('float32')-np.min(ground))/(np.max(ground)-np.min(ground)+1)
                    vis.image(
                        ground,
                        opts=dict(title='ground!', caption='ground.'),
                        win=ground_window,
            ...
            print("data [%d/199/%d/%d] Loss: %.4f loss_d: %.4f loss_var: %.4f loss_dis: %.4f loss_reg: %.4f " % (i, epoch, args.n_epoch,loss.item(),torch.sum(lin).item()/4, \
        ...
        if epoch%check==0:
            ...
            for i_val, (images_val, labels_val,regions,segments) in tqdm(enumerate(valloader)):
                ...
                with torch.no_grad():
                    depth,loss_var,loss_dis,loss_reg = model(images_val,segments_val,1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img_rows', nargs='?', type=int, default=120,
                        help='Height of the input image')
    parser.add_argument('--img_cols', nargs='?', type=int, default=160,
    ...
    parser.add_argument('--resume', nargs='?', type=str, default=r'/home/lidong/Documents/RSCFN/rsn_cluster_nyu2_91_best_model.pkl',
