...
# @Last Modified time: 2018-11-03 15:24:25
import sys
import torch
import visdom
import argparse
...
def train(args):
    ...
    if args.resume is not None:
        ...
        rsn=torch.load('/home/lidong/Documents/RSCFN/rsn_cluster_nyu2_55_0.58963585coarse_best_model.pkl',map_location='cpu')
        model_dict=model.state_dict()  
        #print(model_dict)          
    ...
    #for epoch in range(0, args.n_epoch):
        ...
        for i, (images, labels,regions,segments,image) in enumerate(trainloader):
            ...
            depth,accurate,loss_var,loss_dis = model(images,regions,0,'eval')
            ...
            loss=loss_d+loss_a+loss_var+loss_dis
            ...
            loss_error+=lin.item()
            print("data [%d/%d/%d/%d] Loss: %.4f d: %.4f loss_d:%.4f loss_a:%.4f" % (i,train_len, epoch, args.n_epoch,loss.item(),lin.item(), loss_d.item(),loss_a.item()))
            if (i+1)%4000==0:
                print('testing!')
                model.eval()
                loss_ave=[]
                ...
                for i_val, (images_val, labels_val,regions,segments,image) in tqdm(enumerate(valloader)):
                    ...
                    with torch.no_grad():
                        depth,accurate,_,_ = model(images_val,regions_val,0,'eval')
