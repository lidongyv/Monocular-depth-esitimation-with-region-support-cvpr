...
# @Last Modified time: 2018-11-06 00:54:33
import sys
import torch
import visdom
import argparse
...
def train(args):
    ...
    if args.visdom:
        vis = visdom.Visdom(env='nyu2_cluster_fused_depth')
    ...
    if args.resume is not None:
        ...
        rsn=torch.load('/home/lidong/Documents/RSCFN/location_rsn_cluster_nyu2_77_0.68220705_coarse_best_model.pkl',map_location='cpu')
        model_dict=model.state_dict()  
        #print(model_dict)          
    ...
    #for epoch in range(0, args.n_epoch):
        ...
        for i, (images, labels,regions,segments,image) in enumerate(trainloader):
            ...
            depth,accurate,loss_var = model(images,regions,labels,0,'train',dis=disance)
            disance=0.99*disance+0.01*loss_var.squeeze()
            ...
            loss_error+=lin.item()
            print("data [%d/%d/%d/%d] Loss: %.4f d: %.4f loss_d:%.4f loss_a:%.4f loss_var:%.4f distance:%.4f" % (i,train_len, epoch, args.n_epoch,loss.item(),lin.item(), loss_d.item(),loss_a.item(), \
                torch.sum(loss_var).item(),disance))
        ...
        if epoch%check==0:
            ...
            for i_val, (images_val, labels_val,regions,segments,image) in tqdm(enumerate(valloader)):
                ...
                with torch.no_grad():
                    ...
                    depth,accurate,loss_var= model(images_val,regions_val,labels_val,0,'eval',dis=disance)
            ...
            if error<= best_error:
                ...
                torch.save(state, "fused_{}_{}_{}_{}_coarse_best_model.pkl".format(
        ...
        if epoch%10==0:
            ...
            state = {'epoch': epoch+1,
                     ...
                     'optimizer_state': optimizer.state_dict(), 
                     'error': error,}
            torch.save(state, "fused_{}_{}_{}_coarse_model.pkl".format(
                args.arch, args.dataset,str(epoch)))
            print('save success')
