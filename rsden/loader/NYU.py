...
class NYU(data.Dataset):
    def __init__(self, root, split="train", is_transform=True, img_size=(480,640),task='depth'):
        ...
        if self.split=='train':
            ...
            self.files=os.listdir(self.path)+os.listdir('/home/dataset2/nyu/nyu2/train/')
            #self.files=os.listdir('/home/dataset2/nyu/nyu2/train/')
            #self.files.sort(key=lambda x:int(x[:-4]))
            if len(self.files)<1:
                raise Exception("No files for %s found in %s" % (split, self.path))
            self.l1=len(os.listdir(self.path))
            #self.l2=len(os.listdir('/home/dataset2/nyu/nyu1/train/'))+self.l1
            self.l3=len(os.listdir('/home/dataset2/nyu/nyu2/train/'))+self.l1
